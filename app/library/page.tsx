...
import { FiltersPanel, type FilterState } from "@/components/library/filters-panel";
...
export default function LibraryPage() {
  const { setQueue, playTrack, currentTrack, queue: playerQueue } = usePlayer();
  ...
  const buildQueryParams = useCallback((cursor?: string) => {
    ...
    if (filters.focusMin !== null) params.set("focus_min", filters.focusMin.toString());
    if (filters.focusMax !== null) params.set("focus_max", filters.focusMax.toString());
    if (filters.chillMin !== null) params.set("chill_min", filters.chillMin.toString());
    if (filters.chillMax !== null) params.set("chill_max", filters.chillMax.toString());
    if (filters.tags.length > 0) params.set("tags", filters.tags.join(","));
    if (filters.instrumentation.length > 0) params.set("instrumentation", filters.instrumentation.join(","));
  ...
  // Fetch tracks
  const fetchTracks = useCallback(async (cursor?: string) => {
    setLoading(true);
    try {
  ...
  const handleTrackPlay = useCallback(
    (track: PublicTrack, index: number) => {
      ...
      if (playerQueue.length !== tracks.length || playerQueue[0]?.id !== tracks[0]?.id) {
  ...
  const activeFilters = useMemo(() => {
    ...
    if (filters.artist) active.push({ key: "artist", label: "Artist", value: filters.artist });
    if (filters.genre) active.push({ key: "genre", label: "Genre", value: filters.genre });
    ...
    filters.instrumentation.forEach((instrument) => {
      active.push({ key: `inst-${instrument}`, label: "Instrument", value: instrument });
    });
